...
interface Props {
  ...
  onRequestAccess: () => void; // 触发申请弹窗的函数
}

export const ControlPanel: React.FC<Props> = ({ params, setParams, isUnlocked, onRequestAccess }) => {
  ...
  return (
    <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 h-full overflow-y-auto flex flex-col gap-6">
      ...
      <section>
        ...
        <div className="flex gap-2 mb-2">
          <div className="flex-1">
             <label className="block text-xs font-medium text-slate-600 mb-1">Estart (V)</label>
             <input
          ...
          <div className="flex-1">
             <label className="block text-xs font-medium text-slate-600 mb-1">Eend (V)</label>
             <input
              type="number" step="0.01"
      ...
      <div className="relative pt-2">
        ...
        {/* 内容面板：未解锁时降低交互性 */}
        <div className={!isUnlocked ? "opacity-60 pointer-events-none select-none" : ""}>
        ...
        {/* 【关键】行为拦截透明层：未解锁时覆盖在第3、4板块上方 */}
        {!isUnlocked && (
          <div
            onClick={onRequestAccess}
            className="absolute inset-0 z-30 cursor-pointer hover:bg-slate-900/5 transition-colors rounded-xl flex items-center justify-center group"
          >
            <div className="bg-slate-900 text-white px-4 py-2 rounded-lg text-[10px] font-bold shadow-2xl opacity-0 group-hover:opacity-100 transition-opacity">
